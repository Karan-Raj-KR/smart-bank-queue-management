...
import {
  getQueueStatus, callNextToken, getAnalyticsSummary, getAnalyticsHourly,
  getAnalyticsServiceBreakdown, getWsUrl,
} from '../api'
...
export default function Staff() {
  ...
  // ── FIX 3: WebSocket for teller queue ──────────────────────────────────────
  // Open the WS when the Teller tab is active. On every message, re-fetch the
  // full queue snapshot (the WS payload only carries event + token number, not
  // the full queue array, so a follow-up GET is the right call here).
  useEffect(() => {
    if (activeTab !== 'TELLER') {
      wsRef.current?.close()
      wsRef.current = null
    ...
    ws.onmessage = () => {
      // Any event on this branch means queue state changed — re-fetch
      fetchQueue()
    }

    ws.onerror = (e) => {
      console.warn('Staff WS error', e)
    }

    // Gentle fallback poll (30 s) keeps the view accurate if WS is slow
    const fallbackId = setInterval(fetchQueue, 30000)
    ...
    const fetchAnalytics = async () => {
      ...
      try {
        const [sum, hour, brk] = await Promise.all([
          ...
          getAnalyticsHourly(branchId),
          getAnalyticsServiceBreakdown(branchId),
        ])
        setSummary(sum)
  ...
  const handleCallNext = async () => {
    ...
    try {
      ...
      // WS message will trigger re-fetch automatically; also do it immediately
      const q = await getQueueStatus(branchId)
      setQueueStatus(q)
    } catch (e) {
    ...
    <div className="flex flex-col min-h-[100dvh] bg-surface relative">
      ...
      <div className="flex items-center gap-4 px-6 py-6 border-b border-primary-200 sticky top-0 z-40 bg-white">
        ...
        <div className="flex-1">
          ...
          <p className="text-[10px] uppercase font-bold text-primary-500 tracking-widest mt-1">
            Branch #{branchId}
          </p>
        </div>
      </div>
        ...
        {activeTab === 'TELLER' && (
          <div className="space-y-8 animate-fade-in">
            <div className="flex items-center justify-between">
              ...
              >
                {COUNTERS.map((c) => (
                  <option key={c} value={c}>Counter {c}</option>
                ))}
              </select>
            </div>
              ...
              <div className="space-y-4">
                {(queueStatus?.waiting_queue || []).map((token, index) => (
                  <div
                    ...
                    className="flex justify-between items-center p-5 bg-white rounded-xl shadow-luxury-sm border border-slate-100 hover:shadow-luxury-md transition-shadow group"
                  >
                    <div className="flex items-center gap-6">
                      <div className="w-12 h-12 rounded bg-surface flex items-center justify-center">
                        <span className="serif-heading font-black text-primary-500 text-2xl">{index + 1}</span>
                      </div>
                      <div>
          ...
          <div className="space-y-8 animate-fade-in relative">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="luxury-card flex flex-col items-center py-8">
                ...
                <p className="text-4xl font-black text-primary-900 serif-heading">
                  {summary?.average_wait_time_today || 0}<span className="text-sm text-primary-500 ml-1">m</span>
                </p>
              </div>
              <div className="luxury-card flex flex-col items-center py-8">
                  ...
                  <tbody>
                    {breakdownData?.map((row, i) => (
                      <tr
                        ...
                        className={`border-slate-100 ${i !== breakdownData.length - 1 ? 'border-b' : ''} hover:bg-slate-50 transition-colors`}
                      >
                    ...
                    {(!breakdownData || breakdownData.length === 0) && (
                      <tr>
                        <td colSpan={2} className="py-12 bg-surface text-center text-xs font-bold uppercase tracking-widest text-primary-400">
